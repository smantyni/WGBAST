...
# ~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*
mean1<-read_tsv(str_c(pathM74,"/prg/output/coda_mean_M74_18.txt"), col_names = c("s", "mean1"))
mean2<-read_tsv(str_c(pathM74,"/prg/output/coda_mean2_M74_18.txt"), col_names = c("s", "mean2"))

# change format, otherwise the loops below won't work 
mean1<-as.data.frame(mean1)
mean2<-as.data.frame(mean2)

Years<-c(1985:2016) #spawning years, UPDATE this according to assessment year !!!!
Rivers<-c(1:14)
...
df1<-full_join(df.bugs, dfM74, by=c("YEAR","river"))%>%
