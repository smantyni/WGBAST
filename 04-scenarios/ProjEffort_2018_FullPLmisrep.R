...
#which SR parameterisation is it?
#Model<-"New_SR"
...
# stocknames not used in this file
...
choice<-"MED"   # corresponds to Mps during 20011-2014 period
...
# workflow 2018:
# First, scen 6 was run, finding CoefTrollingF=0.679 to produce ~32k catch
# Next, in scen 1 such Coef1 was found that produces the target catch trolling catch
# being still close to desired 32k.

Coef1<-0.729 
CoefTrollingF<-0.66 # this coef produces the 32k of recr catch when commercial fisheries are removed
...
while(apu==0){
  ...
  ScenE_OLL_TROLLING<-c(3.41,3.41)*CoefTrollingF 
  propTrolling<-(3.41*CoefTrollingF)/((0.42+13.5)*Coef+3.41*CoefTrollingF) # only used for checking
  
  ScenE_CTN_FIN_30<-c(3.9,3.9)*Coef # thousand trapdays
  ScenE_CTN_SWE_30<-c(4.4,4.4)*Coef                 
  ScenE_CTN_FIN_31<-c(5.6,5.6)*Coef       
  ScenE_CTN_SWE_31<-c(5.4,5.4)*Coef       
  ...
  for(loop in 1:10){
    ...
    for(y in (yBreak+1):(yBreak+NumFutYears)){
      ...
      # Use these to save the amounts of salmon that migrate to the river
